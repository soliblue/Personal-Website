...
- Visitor Board is retired. D1 `soli-visitor-board` and `BOARD_HASH_SALT` remain shared rate-limit infrastructure for Messenger and Contact; do not delete them. Migration 0004 drops only visitor entries (the user explicitly requested deletion without a backup).
